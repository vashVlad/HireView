...
function ComparePageInner() {
  ...
  const savedComparisonId = searchParams.get("comparisonId");
  ...
  // On mount: if a comparisonId is in the URL, load the result from DB
  useEffect(() => {
    if (!savedComparisonId || data) return;
    setLoadingFromDb(true);
    fetch(`/api/compare-resumes?comparisonId=${savedComparisonId}`)
      .then((r) => (r.ok ? r.json() : Promise.reject()))
      .then((json) => {
        const rec = json.comparison;
        setData({
          ...
          resumes: [
            ...
            },
            {
              id: null,
              candidateName: existingName,
              fileName: rec.newResumeFilename,
              score: null,
              jobTitle: rec.newResumeRole ?? "New application",
      ...
      .catch(() => setError("Could not load saved comparison."))
  ...
  async function handleCompare() {
    ...
    try {
      ...
      // Update the URL with comparisonId so refresh reloads from DB
      const params = new URLSearchParams(searchParams.toString());
      params.set("comparisonId", String(json.comparisonId));
  ...
  function handleCompareAgain() {
    setData(null);
    // Remove comparisonId from URL so the form shows fresh
    const params = new URLSearchParams(searchParams.toString());
    params.delete("comparisonId");
    router.replace(`/compare?${params.toString()}`);
    // newFile stays pre-populated
  }
        ...
        {loadingFromDb ? (
          ...
          <div className="flex flex-col gap-6">
            <div className="flex items-start justify-between gap-4">
              ...
              {data && (
                <a
                  href={`/compare/${existingId}`}
                  className="shrink-0 rounded-xl border border-zinc-200 px-3 py-2 text-xs font-medium text-zinc-500 transition-colors hover:border-violet-300 hover:text-violet-600 dark:border-zinc-700 dark:text-zinc-400 dark:hover:border-violet-600 dark:hover:text-violet-400"
                >
                ...
                {newFile ? (
                  ...
                  >
                    ...
                    <p className="text-sm font-medium text-zinc-700 dark:text-zinc-200">
                      Drop the new resume here, or click to browse
                    </p>
            ...
            {data && (
              <div className="flex flex-col gap-8 border-t border-zinc-100 pt-8 dark:border-zinc-800">
                {/* Compared resume B label */}
                <div className="flex items-center gap-3 rounded-2xl border border-zinc-200 bg-white px-4 py-3 dark:border-zinc-800 dark:bg-zinc-900">
                  <div className="flex h-9 w-9 shrink-0 items-center justify-center rounded-lg bg-violet-100 text-xs font-semibold uppercase text-violet-600 dark:bg-violet-500/20 dark:text-violet-400">
                    {data.resumes[1]?.fileName.split(".").pop() ?? "B"}
                  </div>
                  <div className="flex flex-col overflow-hidden">
                    <span className="truncate text-sm font-medium text-zinc-700 dark:text-zinc-200">
                      {data.resumes[1]?.fileName}
                    </span>
                    <span className="text-xs text-zinc-400 dark:text-zinc-500">
                      {data.resumes[1]?.jobTitle}
                    </span>
                  </div>
                  <span className="ml-auto shrink-0 rounded-full bg-zinc-100 px-2.5 py-0.5 text-xs font-medium text-zinc-500 dark:bg-zinc-800 dark:text-zinc-400">
                    Resume B
                      ...
                      {data.comparison.changes.map((change, i) => (
                        <div
                          ...
                          className={`grid grid-cols-[16px_1fr_1fr_1fr] gap-x-4 px-4 py-3 text-sm ${
                            i < data.comparison.changes.length - 1
                ...
                <div className="flex items-center gap-3">
                  <button
                    ...
                    className="rounded-xl border border-zinc-200 px-4 py-2 text-sm font-medium text-zinc-600 transition-colors hover:bg-zinc-50 dark:border-zinc-700 dark:text-zinc-300 dark:hover:bg-zinc-900"
                  >
                    Compare again
                  </button>
                  <a
                    href={`/compare/${existingId}`}
                    className="rounded-xl border border-zinc-200 px-4 py-2 text-sm font-medium text-zinc-500 transition-colors hover:border-violet-300 hover:text-violet-600 dark:border-zinc-700 dark:text-zinc-400 dark:hover:border-violet-600 dark:hover:text-violet-400"
                  >
                    View all comparisons
                  </a>
                </div>
