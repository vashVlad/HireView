...
| 2.5.2 | Credibility trajectory overlay | Replaces `assessCredibility.ts`'s field-by-field `rows` with a real second trajectory (cross-ref document run through the same extraction), overlaid on the same graph, diffed in code, mismatches reasoned over by one small scoped AI call | 2.5.1 (both documents need the same checklist to compare against) | **Paused, needs Vlad's call** — audited `assessCredibility.ts` in full 2026-08-17: its `rows` mechanism has extensive hard-won tolerance tuning baked into one AI judgment call (staffing-agency naming, LinkedIn date rounding, 7-rule severity classification). A full rewrite risks that tuning; needs Vlad to pick between the full rewrite vs. a lower-risk alternative (new graph rendered on top of the existing, unchanged `rows` data) before this is touched. |
| 2.5.3 | GitHub/skills corroboration (Gate 4) | Folds a free GitHub API fetch into the *existing* cross-reference check as a third evidence source — not a separate feature/button | 2.5.2 (same file, same call) | **Partially built 2026-08-17** — `lib/githubCorroboration.ts` (username extraction + Voyage-style best-effort fetch), attached as a pure code-side passthrough (`CredibilityAssessment.githubSignal`) shown as a neutral info panel, deliberately NOT fed into `assessCredibility.ts`'s prompt (see 2.5.2's pause reason — same file, same risk). Folding it into the actual AI judgment call is the deferred remainder of this item, blocked on the same 2.5.2 decision. |
| 2.5.4 | Consolidated candidate card | One shared card component replacing 3 separate implementations (`ResultCard.tsx`, Pipeline tab, All Candidates page) | 2.5.2/2.5.3 (so the new tab content is final before the UI merge) | Still blocked — 2.5.2 unresolved. |
...
| 2.5.9 | Global talent search | Natural-language search across every candidate/project via embeddings (Voyage AI) — new infrastructure, no vector/embedding capability exists in this codebase today | Nothing structurally, but the biggest single lift | **Backend built + live-verified 2026-08-17 (commit `5911708`)** — 3 real bugs found and fixed via live testing (degenerate ivfflat index built pre-backfill, miscalibrated `lists` value, unhandled Voyage rate limit). Full backfill (~397 candidates) running in background, ~2.5hr. **CRITICAL: `one-off-reindex-screenings-embedding.sql` must be re-run once the backfill finishes** — it's only been run against 2 test rows so far, search accuracy is unreliable until the index reflects the full dataset. **All Candidates page UI built 2026-08-17 (Cowork)** — semantic search box (`app/candidates/page.tsx`), separate from the existing name filter per the design spec, explicit-submit, stacks with every existing filter (score/date/status/flagged/fraud/blacklist), reorders the list by relevance while active, "Matched: ..." chips on `CandidateCard` for literal term overlap. tsc clean. Not live-tested (needs the reindex-after-backfill step to finish first for results to be meaningful) — this is the full scope of 2.5.9 now, pending a Claude Code verify pass. |
