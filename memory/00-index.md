# Cirot — Memory Vault (Index)

This folder is Cirot's persistent memory. Read this first, every session, before touching code.
...
**3. [[Cirot_Enterprise_Plan]]** — the single source of truth for what gets built, in what order, and why. Contains the feature gate (4 questions every feature must pass), full Phase 1-3 roadmap, UI standards, competitive position, and the Never Build list. Reference this before touching any feature.

**4. [[Cirot_Roadmap]]** — lean feature table with priorities, reuse notes, and logged problem statements. Cross-reference with Enterprise Plan on every decision.
...
**6. [[cirot-enterprise-skill]]** — product governance skill. Apply on every UI decision, every feature scoping question, and every time new ideas are introduced mid-session.
...
| Every feature ships | `state.md` (move to shipped), `Cirot_Impact_Report_Brillio.md` |
| Phase 1.1 ships | `Cirot_Prior_Art_Document.pdf` — add Novel Implementation C (duplicate fingerprinting) same day |
| Any novel implementation | Email vladvashchuk2005@gmail.com with subject "Cirot Prior Art Update — [date]" |
...
Admin-only, isolated module inside Cirot (`app/funnelview/`, `lib/funnelview/`), live data, no CSV/external tools. See [[Cirot_Roadmap]] and [[Cirot_Enterprise_Plan]] for full details. **Merged to main, 2026-07-09 (`34182e0`) — build/typecheck clean, not yet live-tested.** See [[state]] and [[session-log]].

**Phase 2 — Intelligence Layer (30 days)**
See [[Cirot_Enterprise_Plan]] for full details.
...
- `../docs/Cirot_Prior_Art_Document.pdf` — IP protection document, update when novel features ship
- `../docs/Cirot-Dev-Log.docx` — full narrative build history
- `../docs/Cirot-Architecture-Redesign-Phase2-3.docx` — locked nav redesign (shipped)
- `../docs/Cirot-Case-Study.docx`, `../docs/Cirot-Impact-Report.docx` — external-facing writeups
