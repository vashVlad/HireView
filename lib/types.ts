...
// ── Career trajectory (structured) ──────────────────────────────────────────

/**
 * One employment entry in a candidate's career timeline — structured
 * sibling of the freeform careerTrajectory prose string, added 2026-08-17
 * (roadmap 2.5.2, see decisions-log.md's 2026-08-17 entry for the full
 * reasoning). Generated by scoreCandidate.ts (do-not-touch exception) in the
 * SAME call that produces careerTrajectory — describes the identical roles,
 * just as machine-comparable fields instead of prose. Neither field replaces
 * the other: careerTrajectory stays what a recruiter reads for depth,
 * trajectoryEntries is the structured skeleton a real timeline graph and a
 * real code-level diff against a cross-reference document both need — there
 * was no structured per-role data anywhere in this codebase before this.
 */
export interface TrajectoryEntry {
  company: string;
  title: string;
  employmentType: "full-time" | "contract" | "unknown";
  /** YYYY-MM if the source gives a month, YYYY if only a year is known. */
  startDate: string;
...
/**
 * One row of a structured employment-history comparison between a
 * candidate's own trajectoryEntries and a cross-reference document's
 * trajectoryEntries — added 2026-08-17 (roadmap 2.5.2), replacing CredibilityRow's
 * old employment-comparison coverage (CredibilityRow now only ever carries
 * education rows going forward — see CredibilityAssessment.rows).
 *
 * Built in two stages, matching the 2026-08-14 plan's original "code-level
 * diff, not AI" design:
 *   1. lib/matchTrajectoryEntries.ts (pure, deterministic, zero AI cost)
 *      pairs entries from both arrays by date-range overlap, computes
 *      fieldDiffs for each pair, and surfaces any cross-reference entry with
 *      no resume-side counterpart at all as kind: "undisclosed". A paired
 *      row with zero fieldDiffs is definitionally a match — status is set
 *      directly by this step, no AI involvement needed.
 *   2. Only rows this deterministic step could NOT resolve to a confident
 *      match (kind: "undisclosed", or any fieldDiffs entry true) go to one
 *      small scoped AI call (lib/assessCredibility.ts) that applies the
 *      SAME tolerance tuning CredibilityRow's severity field used to apply
 *      per-field (staffing-agency naming, title phrasing, ~2-month date
 *      rounding, undisclosed-overlap-vs-harmless-gap) and writes the final
 *      status/severity/note.
 */
export interface TrajectoryComparisonRow {
  /** Undefined only for a kind: "undisclosed" row. */
  resumeEntry?: TrajectoryEntry;
  /**
   * Undefined when the resume lists a role the cross-reference document
   * doesn't mention at all — deliberately never surfaced as its own row
   * (same "normal resume trimming, not concealment" reasoning the old
   * schema's undisclosed-employment guidance used for the REVERSE
   * direction) — this field is only ever undefined together with
   * resumeEntry being present, i.e. kind: "paired" rows always have a
   * crossRefEntry too. A kind: "undisclosed" row always has this populated
   * (it exists because the cross-reference mentions it) and resumeEntry
   * undefined.
   */
  crossRefEntry?: TrajectoryEntry;
  kind: "paired" | "undisclosed";
  /** Deterministic, set by the code-level diff — which fields actually differ for a "paired" row. Absent for "undisclosed" (nothing to compare against). */
  fieldDiffs?: { company: boolean; title: boolean; dates: boolean };
  status: "match" | "discrepancy" | "cannot_verify";
  /** Required when status is "discrepancy" — same material/minor bar CredibilityRow's severity field used. */
  severity?: "material" | "minor";
  /** Required when status is "discrepancy" — one short factual sentence, same bar as CredibilityRow.note. */
  note?: string;
}
...
export interface CredibilityAssessment {
  /**
   * As of 2026-08-17 (roadmap 2.5.2), only ever populated for EDUCATION rows
   * going forward — employment comparisons moved to trajectoryComparison
   * below, a structured diff instead of a field-by-field AI judgment on
   * every row. Assessments saved BEFORE this date still have real employment
   * rows here and nothing in trajectoryComparison — UI must check
   * trajectoryComparison first and only fall back to treating rows as
   * employment data when it's absent, to render older saved results
   * correctly. Always an array (possibly empty), never absent — same as
   * before this change.
   */
  rows: CredibilityRow[];
  /**
   * New, 2026-08-17 (roadmap 2.5.2) — structured employment-history
   * comparison, see TrajectoryComparisonRow. Absent (not just empty) for
   * assessments run before this date, or when the candidate's own
   * trajectoryEntries doesn't exist yet (pre-2026-08-17 screening, never
   * regenerated) — falls back to the old rows-based Flags/Matches view in
   * that case, same graceful-degradation convention every deferred field in
   * this codebase already follows.
   */
  trajectoryComparison?: TrajectoryComparisonRow[];
...
export interface CandidateResult {
  ...
  /**
   * Structured sibling of careerTrajectory, added 2026-08-17 (roadmap 2.5.2,
   * do-not-touch exception — see decisions-log.md's 2026-08-17 entry). Same
   * "generated in the same call, straight off this result" reasoning as
   * currentCompany/currentTitle below. See lib/types.ts's TrajectoryEntry.
   */
...
export interface ScreeningRecord {
  ...
  /**
   * Structured sibling of careerTrajectory, added 2026-08-17 (roadmap 2.5.2)
   * — see lib/types.ts's TrajectoryEntry and CandidateResult.trajectoryEntries
   * for the full "why." Deliberately NOT in the shared SCREENING_COLUMNS
   * select — same deferred pattern as currentCompany below, see
   * supabase-migration-trajectory-entries.sql. Undefined for any screening
   * saved before this date (never regenerated) or before the migration runs.
   */
  trajectoryEntries?: TrajectoryEntry[];
  recommendation: Recommendation | null;
  status: CandidateStatus;
